...
struct LockedWebView: UIViewRepresentable {
        ...
        UIScrollViewDelegate
    {
            ...
            themeObservation = webView.observe(\.themeColor, options: [.new]) {
                ...
                DispatchQueue.main.async {
                    if let uiColor = change.newValue as? UIColor {
                        ...
                        WebRefreshManager.shared.currentThemeColor = Color(
                            uiColor: uiColor
                        )
                    }
                }
        ...
        func userContentController(
            ...
            if message.name == "bluetooth" {
                ...
                BLEManager.shared.handleWebBluetoothMessage(
                    id: id,
                    method: method,
                    args: args
                )
            ...
            if message.name == "consoleLog" {
                if let body = message.body as? [String: Any] {
                    ...
                    // LockedWebview.swift
                    logger.log(
                        "[JS:\(level, privacy: .public)] \(text, privacy: .public)"
                    )
                }
                return
            }
        ...
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!)
        {
            ...
            logger.log(
                "[LockedWebView] Page loaded: \(webView.url?.absoluteString ?? "?")"
            )
        }
        ...
        func webView(
            ...
            webView.scrollView.refreshControl?.endRefreshing()
            logger.log(
                "[LockedWebView] Navigation error: \(error.localizedDescription)"
            )
        }
        ...
        func webView(
            ...
            webView.scrollView.refreshControl?.endRefreshing()
            logger.log(
                "[LockedWebView] Provisional navigation error: \(error.localizedDescription)"
            )
        }
    }
    func updateUIView(_ uiView: WKWebView, context: Context) {
        // Dynamically update the color when the observable property changes
        context.coordinator.topThemeView?.backgroundColor = UIColor(
            refreshManager.currentThemeColor
        )
    }
    func makeUIView(context: Context) -> WKWebView {
        ...
        if let jsURL = Bundle.main.url(
            forResource: "WebBluetooth",
            withExtension: "js"
        ),
            let src = try? String(contentsOf: jsURL, encoding: .utf8)
        {
            ...
            assertionFailure(
                "WebBluetooth.js not found — add to Copy Bundle Resources"
            )
        }
...
struct WebView: View {
    ...
    private var lockedURL: URL {
        let base =
            !Settings.shared.webURL.isEmpty
            ? Settings.shared.webURL : "banglejs.com/apps"
