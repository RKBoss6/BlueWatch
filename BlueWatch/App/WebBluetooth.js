...
function processWriteQueue() {
    ...
    _call('writeValue', {charId : item.charId, value : item.value})
...
window.__bluetoothNotify = function(charId, byteArray) {
    ...
    if (!list || list.length === 0) {
        if (!_charBuffer[charId])
            _charBuffer[charId] = [];
        if (_charBuffer[charId].length < 128)
            _charBuffer[charId].push(ev);
        return;
    }
    for (var i = 0; i < list.length; i++) {
        try {
            ...
            console.error('[WB] listener error', e);
        }
    }
};
// ── Disconnect dispatch ─────────────────────────────────────────────────────
...
window.__bluetoothDisconnected = function() {
    ...
    Object.keys(_deviceListeners).forEach(function(did) {
        (_deviceListeners[did] || []).forEach(function(fn) {
            try {
                fn(ev);
            } catch (e) {
            }
...
function makeCharacteristic(charId, uuid, propsRaw, serviceRef) {
    ...
    char.addEventListener = function(type, fn) {
        if (type !== 'characteristicvaluechanged')
            return;
        if (!_charListeners[charId])
            _charListeners[charId] = [];
            ...
            setTimeout(function() {
                for (var i = 0; i < toFlush.length; i++) {
                    try {
                        ...
                        console.error('[WB] flush error', e);
                    }
                }
            }, 0);
        }
    ...
    char.removeEventListener = function(type, fn) {
        if (!_charListeners[charId])
            return;
        _charListeners[charId] =
            _charListeners[charId].filter(function(f) { return f !== fn; });
    };
    char.startNotifications =
        function() { return _call('startNotifications', {charId : charId}); };
    char.stopNotifications =
        function() { return _call('stopNotifications', {charId : charId}); };
    char.writeValue = function(b) {
        ...
        for (var offset = 0; offset < bytes.length; offset += _chunkSize) {
            ...
            promises.push(
                queueWrite(charId, Array.from(bytes.slice(offset, end))));
        }
...
function makeService(serviceId, uuid, deviceRef) {
    ...
    service.getCharacteristic = function(charUUID) {
        ...
        return _call('getCharacteristic',
                     {serviceId : serviceId, charUUID : full})
            .then(function(r) {
                return makeCharacteristic(r.charId, full, r.props, service);
            });
    };
    service.getCharacteristics = function(charUUID) {
        return service.getCharacteristic(charUUID).then(function(
            c) { return [ c ]; });
    };
    return service;
}
function makeGATTServer(deviceId, deviceRef) {
    ...
    server.getPrimaryService = function(serviceUUID) {
        ...
        return _call('getPrimaryService',
                     {deviceId : deviceId, serviceUUID : full})
            .then(function(
                r) { return makeService(r.serviceId, full, deviceRef); });
    };
...
function makeDevice(deviceId, name) {
    ...
    device.addEventListener = function(type, fn) {
        if (!_deviceListeners[deviceId])
            _deviceListeners[deviceId] = [];
    ...
    device.removeEventListener = function(type, fn) {
        if (!_deviceListeners[deviceId])
            return;
        _deviceListeners[deviceId] =
            _deviceListeners[deviceId].filter(function(f) { return f !== fn; });
    };
    return device;
}
// ── UUID resolver ───────────────────────────────────────────────────────────
...
function resolveUUID(uuid) {
    if (!uuid)
        return uuid;
    if (typeof uuid === 'string' && uuid.indexOf('-') !== -1)
        return uuid.toLowerCase();
    if (typeof uuid === 'number') {
        return '0000' + uuid.toString(16).padStart(4, '0') +
               '-0000-1000-8000-00805f9b34fb';
    }
    if (typeof uuid === 'string') {
        var n = parseInt(uuid, 16);
        if (!isNaN(n))
            return '0000' + n.toString(16).padStart(4, '0') +
                   '-0000-1000-8000-00805f9b34fb';
