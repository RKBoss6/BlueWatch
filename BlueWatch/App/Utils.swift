    ...
    static func clearAllData() {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            do {
                ...
                logger.log(
                    "Failed to delete SwiftData records: \(error.localizedDescription)"
                )
            }
        }
    }
...
enum DataService {
    static func addDataPointInBackground(
        timestamp: Date,
        value: Double,
        type: DataType,
        alwaysSave: Bool
            ...
            if !alwaysSave {
                ...
                if let lastPoint = try? context.fetch(limitedDescriptor).first {
                    if lastPoint.value == value
                        && Date().timeIntervalSince(lastPoint.timestamp) < 60
                            * 9 /*Only skip a save if its less than 9 minutes apart from the last one.*/
                    {
                        logger.log(
                            "Skipping save: Value for \(type.rawValue, privacy: .public) hasn't changed (\(value))"
                        )
