...
public class Settings: ObservableObject {
    ...
    private var showFindPhoneNotificationKey: String =
        "showFindPhoneNotification"
    @Published public var showFindPhoneNotification: Bool {
        didSet {
            LocalStorage.set(
                showFindPhoneNotification,
                forKey: showFindPhoneNotificationKey
            )
        }
    }
    ...
    @Published public var pullToRefreshWebView: Bool {
        didSet {
            LocalStorage.set(
                ...
                forKey: pullToRefreshWebViewKey
            )
        }
    }
    ...
    init() {
        ...
        sendToHealthKit =
            LocalStorage.getBool(forKey: sendToHealthKitKey) ?? true
        showFindPhoneNotification =
            LocalStorage.getBool(forKey: showFindPhoneNotificationKey) ?? true
        deviceName =
            LocalStorage.getString(forKey: deviceNameKey)
            ?? ViewModel.shared.savedDevice
        weatherRateLimit = Int(
            LocalStorage.getNumber(forKey: weatherRateLimitKey) ?? 10
        )
        locationRateLimit = Int(
            LocalStorage.getNumber(forKey: locationRateLimitKey) ?? 10
        )
        optimizedBtChunks =
            LocalStorage.getBool(forKey: optimizedBtChunksKey) ?? true
        showHrThumb = LocalStorage.getBool(forKey: showHrThumbKey) ?? true
        showBatteryThumb =
            LocalStorage.getBool(forKey: showBatteryThumbKey) ?? true
        showStepsThumb = LocalStorage.getBool(forKey: showStepsThumbKey) ?? true
        showActiveCalThumb =
            LocalStorage.getBool(forKey: showActiveCalThumbKey) ?? true
        showRestingCalThumb =
